...
public class DiamondVault implements Economy {
	...
	public EconomyResponse depositPlayer(OfflinePlayer player, double amount) {
		if(!hasAccount(player)) return new EconomyResponse(0, 0, EconomyResponse.ResponseType.FAILURE, plugin.constructString("config.failedFindPlayer", amount, player.getPlayer().getName()));
	...
	public EconomyResponse withdrawPlayer(OfflinePlayer player, double amount) {
		if(!hasAccount(player)) return new EconomyResponse(0, 0, EconomyResponse.ResponseType.FAILURE, plugin.constructString("config.failedFindPlayer", amount, player.getPlayer().getName()));
		if(!has(player, amount)) return new EconomyResponse(0, 0, EconomyResponse.ResponseType.FAILURE, plugin.constructString("config.insufficientBalanceMsg", amount, player.getPlayer().getName()));
