...
public class BankCommand implements CommandExecutor {
	...
	private void baltop(Player p) {
		...
		p.sendMessage(this.plugin.constructString("config.balTopMsg", 0, p.getName()));

		for (int i = 0; i < maxBalances; i++) {
			Player bal = balTop.poll();
			p.sendMessage(String.format("%d: %s - %s", i + 1, bal.getName(), plugin.econ.getBalance(bal)));
		}
	...
	private void addBal(Player p, String[] args) {
		...
		p.sendMessage(plugin.constructString("config.addBalanceMsg", amount, player.getName()));
	}
	...
	private void pay(Player p, String[] args) {
		...
		if(amount > plugin.econ.getBalance(p)) {
			p.sendMessage(plugin.constructString("config.insufficientBalanceMsg", amount, p.getName()));
			return;
		}
	...
	private void deposit(Player p, String[] args) {
		...
		if(diamonds == 0) {
			p.sendMessage(plugin.constructString("config.insuffcientDiamondsMsg", 0, p.getName()));
			return;
		}
	...
	private void balance(Player p, String[] args) {
		...
		p.sendMessage(plugin.constructString("config.balanceMsg", balance, p.getName()));
	}
}
