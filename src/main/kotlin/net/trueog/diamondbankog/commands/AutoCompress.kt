...
internal class AutoCompress : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        if (DiamondBankOG.redis.getValue("diamondbankog:${sender.uniqueId}:autocompress") == "true") {
            ...
            sender.sendMessage(
                DiamondBankOG.mm.deserialize(
                    "${Config.prefix}<reset>: <green>Successfully turned auto-compress <red>off<green>."
                )
            )
        } else {
            ...
            sender.sendMessage(
                DiamondBankOG.mm.deserialize("${Config.prefix}<reset>: <green>Successfully turned auto-compress on.")
            )
        }
        return true
    }
