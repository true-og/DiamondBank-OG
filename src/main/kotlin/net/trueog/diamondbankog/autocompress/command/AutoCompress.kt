...
internal class AutoCompress : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        if (redis.getValue("diamondbankog:${sender.uniqueId}:autocompress") == "true") {
            ...
            sender.sendMessage(
                mm.deserialize("${config.prefix}<reset>: <green>Successfully turned auto-compress <red>off<green>.")
            )
        } else {
            if (redis.getValue("diamondbankog:${sender.uniqueId}:autodeposit") == "true") {
                sender.sendMessage(
                    mm.deserialize(
                        "${config.prefix}<reset>: <#FFA500>You cannot have both auto-compress and auto-deposit on at the same time, so auto-deposit has been turned <red>off<#FFA500>."
                    )
                )
                redis.setValue("diamondbankog:${sender.uniqueId}:autodeposit", "false")
            }
            redis.setValue("diamondbankog:${sender.uniqueId}:autocompress", "true")
            sender.sendMessage(mm.deserialize("${config.prefix}<reset>: <green>Successfully turned auto-compress on."))
        }
        return true
    }
}
