...
internal class Events : Listener {
    ...
    fun onPlayerInteract(event: PlayerInteractEvent) {
        ...
        if (DiamondBankOG.economyDisabled) {
            event.isCancelled = true
            event.player.sendMessage(DiamondBankOG.mm.deserialize("${Config.prefix}<reset>: <red>You cannot interact with any economy-related items while the economy is disabled."))
            return
        }
    ...
    fun onPlayerInteractEntity(event: PlayerInteractEntityEvent) {
        ...
        if ((itemTypeInMainHand != Material.DIAMOND && itemTypeInMainHand != Material.DIAMOND_BLOCK && itemTypeInMainHand != Material.SHULKER_BOX && !(itemTypeInMainHand == Material.PRISMARINE_SHARD && itemStackInMainHand.persistentDataContainer.has(
                Shard.namespacedKey
            )) && (itemTypeInOffHand != Material.DIAMOND && itemTypeInOffHand != Material.DIAMOND_BLOCK && itemTypeInOffHand != Material.SHULKER_BOX && !(itemTypeInOffHand == Material.PRISMARINE_SHARD && itemStackInOffHand.persistentDataContainer.has(
                Shard.namespacedKey
            ))))
        ) {
            return
