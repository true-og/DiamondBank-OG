...
object PlayerPrefix {
    ...
    fun legacyToMm(text: String): String {
        return legacyRegex.replace(text) {
            legacyToMmMap[it.groupValues[1].lowercase()] ?: it.value
        }
    }

    suspend fun getPrefix(uuid: UUID): String {
        val user =
            DiamondBankOG.luckPerms.userManager.getUser(uuid) ?: DiamondBankOG.luckPerms.userManager.loadUser(uuid)
                .await()
