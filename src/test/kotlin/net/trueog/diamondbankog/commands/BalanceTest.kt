...
class BalanceTest {
    ...
    @Test
    @DisplayName("Balance with no permission should fail")
    fun balanceNoPermission() = runTest {
        every { player.hasPermission("diamondbank-og.balance") } returns false

        val balance = Balance(config, balanceManager, mm, scope)
        balance.onCommand(player, command, "balance", arrayOf())
        waitForCoroutines(scope)

        assertAll(
            { verify { player.hasPermission("diamondbank-og.balance") } },
            {
                verify {
                    player.sendMessage(
